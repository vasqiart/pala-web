#!/usr/bin/env node
/**
 * PALANTIR（COMPANY）ページ背景用画像 16 枚を _incoming/paradog/palantir_page_background から検出し、
 * public/paradog/company_bg/ に palantir_page_001.png ... 016.png として配置する。
...
const INCOMING_DIR = path.join(ROOT, "_incoming", "paradog", "palantir_page_background");
const OUT_DIR = path.join(ROOT, "public", "paradog", "company_bg");
...
async function main() {
  ...
  if (!fs.existsSync(INCOMING_DIR)) {
    console.error("ERROR: _incoming/paradog/palantir_page_background が存在しません。");
    process.exit(1);
  }
  ...
  if (files.length !== REQUIRED_COUNT) {
    ...
    if (files.length > 0) {
      ...
      console.error("_incoming/paradog/palantir_page_background に画像を 16 枚入れてから再度実行してください。");
    }
    process.exit(1);
  }
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    const outName = `palantir_page_${num}.png`;
  ...
  console.log("\n出力先 public/paradog/company_bg に 16 枚の PNG を生成しました。");
}
